...
const FAQS: { q: string; a: React.ReactNode }[] = [
  ...
  {
    q: "What if the bond loses money?",
    a: (
      <>
        SY is a bond position, so a bond default or loss is an SY loss. The protocol prices the
        shortfall instead of freezing up: PT redemptions cap at each holder&rsquo;s fair share
        (everyone shares the loss equally, so there is no bank-run race), and PT is paid before YT.
        See <Link href="/docs/settlement">Settlement and maturity</Link>.
      </>
    ),
  },
  {
    ...
    a: (
      <>
        The contracts cannot be upgraded, paused, or used to seize holder funds. Their configured
        admins can update bounded swap, claimed-yield, and order-taker fees; those limits are
        enforced on-chain and every update emits an event. See{" "}
        <Link href="/docs/security">Security and risks</Link>.
      </>
    ),
  },
